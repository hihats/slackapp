...
# Run the application
docker run -v $(pwd)/output:/app/output slackapp \
...
python wordcloud.py --token YOUR_TOKEN --channel CHANNEL_ID --keyword KEYWORD
```
...
docker run --volume $PWD:/app slackapp posts_with_my_reactions.py --token $SLACK_TOKEN --days $NUMDAYS --output posts_with_my_reactions.json
```
...
docker run --volume $PWD:/app slackapp message_reactions.py --token $SLACK_TOKEN --channel $CHANNEL_ID --message $MESSAGE_TIMESTAMP --include-user-details --output reactions_to_$MESSAGE_TIMESTAMP.json
```

### Available Command Line Arguments

- `--token`: Slack API token (required)
- `--channel`: Slack channel ID to search (required)
- `--keyword`: Search keyword (required)
- `--days`: Number of days to search back (default: 30)
- `--output`: Output filename (default: wordcloud.png)
...
7. Save output to `/app/output/` directory
